...
# NIGHTJAR_IMAGE_UNSEED=1 removes the endpoint instead (used when the OpenAI key is
# removed from BYOK settings, so image gen stops pointing at a deleted key).
unseed = os.environ.get("NIGHTJAR_IMAGE_UNSEED") == "1"
...
try:
    if unseed:
        ...
        if ep is not None:
            db.delete(ep)
            db.commit()
            print(f"[seed] removed image endpoint: name={name!r}")
        else:
            print(f"[seed] no image endpoint named {name!r} to remove")
    else:
        ep = (
            db.query(ModelEndpoint)
            .filter(ModelEndpoint.model_type == "image", ModelEndpoint.name == name)
