...
PR B (Odysseus removal): the `web_search` tool that used to live here moved to
`phase2-mcp/websearch_server.py`. It never used Odysseus — it only died with this
module because `from src.deep_research import DeepResearcher` below runs at import
time. This file now holds `deep_research` ONLY, and is itself replaced in PR F.
"""
...
from research_backend import resolve_research_llm  # pure selector; now lives in phase2-mcp/
