...
- `voice.user.transcript.final` records Voice/STT completion only; it does not authorise durable work, effects, or durable memory candidates.
- The Conversation Service emits `conversation.turn.finalized` only after it authenticates, normalises, accepts, and persists the final user turn.
...
Voice emits `voice.user.transcript.final` when STT produces the final transcript. The user message becomes canonical only when the Conversation Service:
...
6. the message is authenticated, normalised, accepted, and persisted.

The Conversation Service then emits `conversation.turn.finalized`. The earlier Voice transcript event alone is not authority for durable work, consequential effects, or durable memory-candidate extraction.
...
The event names in this adapter surface are adapter-local callbacks. The adapter translates `user.transcript.final` into the canonical Voice event `voice.user.transcript.final`; only the Conversation Service may subsequently emit `conversation.turn.finalized`.
...
- Voice emits `voice.user.transcript.final` for the completed STT result.
- The Conversation Service validates and persists that result as one canonical user message, then emits `conversation.turn.finalized` to signal that the canonical commit is complete.
- The Voice transcript-final event alone cannot create a durable task, effect, or durable memory candidate.
...
The same authority boundary applies after STT finalisation: `voice.user.transcript.final` may update temporary presentation and speculative read-only work, but only `conversation.turn.finalized` may admit canonical cross-system processing.
...
`operation_kind` and `execution_mode` are structured routing hints proposed by the realtime model, not trusted authority. JUNE validates or recomputes them against the trusted capability contract together with side effects, `risk_tier`, required permission, retry class, and verification requirements.

The values `local_write` and `protected_write` are not operation kinds: both use `operation_kind = write`, with JUNE normally deriving R1 for a local reversible write and R2/R3 for a protected write. `long_running_read` is represented by `operation_kind = read` plus `execution_mode = long_running`; `long_running` is an execution mode, not an operation kind.

Security-critical IDs, authenticated user identity, policy context, capability tokens, and the authoritative `risk_tier` are attached by JUNE—not generated by the model. A provider/model cannot supply or lower R0-R3, and Memory cannot raise a risk tier or permission ceiling.
...
Stable partial speech may trigger speculative read-only retrieval. `voice.user.transcript.final` confirms the STT result but still does not authorise a durable write. Final canonical retrieval and cross-system processing use `conversation.turn.finalized`.
...
For voice-derived user input, durable memory-candidate extraction begins only after `conversation.turn.finalized`. A separately authorised scheduled/system trigger may process otherwise eligible canonical evidence but cannot bypass Memory source or write policy. Extraction is asynchronous and cannot delay first audio.
...
- `voice.user.transcript.final` remains a Voice/STT result; the Conversation Service validates and persists the resulting user message as canonical, then emits `conversation.turn.finalized` to signal that the commit is complete;
...
Provider transcripts and model tool calls are untrusted proposals. `june_delegate` arguments, including `operation_kind` and `execution_mode`, are validated or recomputed against a trusted capability schema and policy before work admission. JUNE derives side effects, risk tier, permission, retry, and verification requirements.
...
`voice.user.transcript.final` is emitted by Voice. `conversation.turn.finalized` is emitted by the Conversation Service after the user turn is authenticated, normalised, accepted, and persisted. The former documentation name `voice.turn.committed` is deprecated and new code must not emit it.
...
```text
VT-001 Conversation Service persists exactly one canonical user message after voice.user.transcript.final, then conversation.turn.finalized signals the completed commit
VT-002 partial transcript creates no durable message
VT-003 neither partial transcript nor voice.user.transcript.final directly creates task, effect, or durable memory
...
- memory retrieval is bounded and speculative reads are allowed; for voice-derived user input, durable writes use only eligible evidence admitted after `conversation.turn.finalized`, while separately authorised scheduled/system processing cannot bypass Memory policy;
