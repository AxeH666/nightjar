...
- **`cad` agent:** `"*":"deny"` + 15 `allow` + 5 `ask` (20 reachable of 38) ([`opencode.json:69-96`](engine-workspace/opencode.json#L69-L96)). **Mechanical reuses this verbatim** — no new agent, no new MCP.
- **`cad-build123d` MCP:** phase-cad venv python + `cad_mcp_server.py`, `BUILD123D_IN_PROCESS=1`, 180 s timeout ([`opencode.json:165-171`](engine-workspace/opencode.json#L165-L171)).
