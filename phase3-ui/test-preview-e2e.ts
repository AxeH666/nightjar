...
// fileURLToPath (not URL.pathname) — on Windows .pathname yields "/C:/dev/..." which is
// not a native path, and normalizeRel() would then fail to relate tool paths to it.
const WS = process.env.NIGHTJAR_WORKSPACE || fileURLToPath(new URL("../engine-workspace", import.meta.url))
