...
StructuredTool = object  # NIGHTJAR: langchain removed (adapter dropped)
    ...
    **Threading model**: Playwright's sync API is bound to the OS thread
    ...
    def _pw_loop(self) -> None:
        ...
        try:
            ...
            channel = None  # NIGHTJAR: force bundled chromium

            self._pw = sync_playwright().start()

            launch_kwargs: dict[str, Any] = {
                "user_data_dir": str(_PROFILE_DIR),
                "headless": True,  # NIGHTJAR: headless (no display)
                "viewport": _VIEWPORT,
                "bypass_csp": True,
                "args": [
